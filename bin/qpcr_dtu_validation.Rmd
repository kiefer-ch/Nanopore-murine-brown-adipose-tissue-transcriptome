...
```{r}
dtu <- read_csv(snakemake@input[["signif"]])
...
plot.transcripts <- function(gene_id) {
    ...
    sTrack <- SequenceTrack(snakemake@input[["genome"]],
        genome = "mm10", chromosome = gene_info$TXCHROM)
    ...
    plotTracks(list(gtrack, illumina_warm, illumina_cold, teloprime_warm, teloprime_cold, cdna_warm, cdna_cold, atrack),
        just.group = "above",
        ...
        fontsize = 12, cex.axis = .4,
        min.height = 0, coverageHeight = 0.08, minCoverageHeight = 0)
}
```

```{r}
plot.transcripts(pvalues$ensembl_gene_id_version[[1]])
```
