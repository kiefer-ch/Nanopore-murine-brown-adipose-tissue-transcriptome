...
```{r}
df <- list(illumina_tx, teloprime_tx) %>% 
    set_names("illumina", "teloprime") %>% 
    map(get_groupedTxLength_byBiotype) %>% 
...
pl2
saveRDS(pl2,
    file.path(snakemake@params[["fig_folder"]], "transcriptDetection_by_datasetLength.rds"))
```
