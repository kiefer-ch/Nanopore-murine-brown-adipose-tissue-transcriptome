...
Cutoff is at least 3 counts in the teloprime dataset (as in Soneson et al., 2019)
and 10 in the illumina dataset.
...
```{r vennTranscripts}
library("nVennR")
plotVenn(list(Teloprime = expressedInONT, Illumina = expressedInIllumina),
    nCycles = 2000)
```


```{r densityAfterFilter, dependson="filterNotExpressed"}
pl_countsIllumina <- df_expressedInBoth %>% 
    ggplot(aes(counts_illumina)) +
    geom_density(aes(colour = sample)) +
    scale_x_continuous(
        breaks = c(log2(c(1, 10, 100, 1000, 10000)), max(df$counts_illumina, na.rm = TRUE)),
...
pl_countsOnt <- df_expressedInBoth %>% 
    ggplot(aes(counts_ont)) +
    geom_density(aes(colour = sample)) +
    scale_x_continuous(
        breaks = c(log2(c(1, 10, 100, 1000, 10000)), max(df$counts_ont, na.rm = TRUE)),
...
dir.create("../res/comparisons", showWarnings = FALSE)
df %>% 
    write_csv("../res/comparisons/countMatrix_transcripts.csv.gz")
```
...
df_genes %>% 
    write_csv("../res/comparisons/countMatrix_genes.csv.gz")
```
