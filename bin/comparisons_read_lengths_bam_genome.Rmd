...
get_overlap <- function(same_reference, pos, pos2, ref_pos) {
    ...
    same_reference &
        (any(findInterval(c(pos, pos2), unlist(ref_pos), rightmost.closed = TRUE) == 1L) |
            any(findInterval(unlist(ref_pos), sort(c(pos, pos2)), ), rightmost.closed = TRUE) == 1L)
}
