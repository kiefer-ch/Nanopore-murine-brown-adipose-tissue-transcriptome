---
title: "nanopore iBAT, Illumina reads gene level, all samples"
...
# import data
dds <- readRDS("../data/dds_gencode.vM22_gene_ont.rds")
```
...
assay(rld) %>%
    ...
    annotate_cm() %>% 
    write_csv(file.path(res_folder, "genelevel_all_cm_rld.csv.gz"))

# without
assay(ntd) %>% 
    as_tibble(rownames = "gene_id_ens") %>% 
    annotate_cm() %>% 
    write_csv(file.path(res_folder, "genelevel_all_cm_ntd.csv.gz"))

# tpm
tpm %>% 
    as_tibble(rownames = "gene_id_ens") %>% 
    annotate_cm() %>% 
    write_csv(file.path(res_folder, "genelevel_all_cm_tpm.csv.gz"))

# raw counts
counts(dds, normalized = FALSE) %>% 
    as_tibble(rownames = "gene_id_ens") %>% 
    annotate_cm() %>% 
    write_csv(file.path(res_folder, "genelevel_all_cm_cts.csv.gz"))
```
...
```{r runDESeq, dependson="importData"}
# set reference
...
resLFC %>% 
    ...
    write_csv(gzfile(file.path(res_folder, "genelevel_all_de.csv.gz")))
