---
title: "DTU, Illumina reads transcript level, only samples used for ont too"
...
```{r createTxdb}
txdb <- GenomicFeatures::makeTxDbFromGFF("../annotation/annotation.gtf",
...
```{r importScaledTpm}
cts <- readRDS("../data/scaledTPM.rds")
...
dir.create("../res/dtu_ont", showWarnings = FALSE)
...
res_gene %>% 
    ...
    dplyr::rename(ensembl_gene_id_version = "gene_id") %>% 
    left_join(annotation_gene, by = "ensembl_gene_id_version") %>% 
    write_csv("../res/dtu_ont/genelevel_drimseq_dtu.csv.gz")
...
res_tx %>% 
    ...
    as_tibble() %>% 
    arrange(adj_pvalue) %>% 
    write_csv("../res/dtu_ont/txlevel_drimseq_dtu.csv.gz")
```
...
res_stageR %>% 
    write_csv("../res/dtu_ont/stageR_drimseq_dtu.csv.gz")
...
```{r include=FALSE}
plotProportions(dmds, gene_id = "ENSMUSG00000039205.16",
    group_variable = "condition_temp", plot_type = "boxplot1") +
