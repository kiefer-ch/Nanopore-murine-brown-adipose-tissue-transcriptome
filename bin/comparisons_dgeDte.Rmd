...
tx <- snakemake@input$tx_counts %>% 
    ...
    select(id, dataset, cold, warm) %>% 
    mutate(fold_change = cold/warm,
           base_mean = rowMeans(select(., cold, warm))) %>% 
    select(-cold, -warm)
...
df %>%
    ...
    upset(nsets = 6, keep.order = TRUE,
        nintersects = 15,
        order.by = "freq",
        sets = c("illumina_gene", "illumina_transcript", "teloprime_gene", "teloprime_transcript", "cdna_gene", "cdna_transcript"),
...
```{r}
df_plot <- gene %>% 
    tidyr::pivot_wider(names_from = dataset, values_from = c(fold_change, base_mean)) %>% 
    mutate_if(is.numeric, tidyr::replace_na, 0) %>% 
    full_join(dge, by =c ("id" = "ensembl_gene_id_version")) %>% 
    mutate(across(ends_with("_gene"), ~ . <= snakemake@params$cutoff)) %>% 
    left_join(biotype_groups, by = c("gene_biotype" = "transcript_biotype"))
```
...
telo_exp <- df_plot %>%
    mutate(signif = case_when(
            illumina_gene & teloprime_gene  ~ "both",
            illumina_gene                   ~ "illumina",
            teloprime_gene                  ~ "longRead",
            TRUE                            ~ "n.s.")) %>% 
    filter(signif != "n.s.") %>% 
...
telo_exp <- df_plot %>%
    mutate(signif = case_when(
            illumina_transcript & teloprime_transcript  ~ "both",
            illumina_transcript                   ~ "illumina",
            teloprime_transcript                  ~ "longRead",
            TRUE                            ~ "n.s.")) %>% 
    filter(signif != "n.s.") %>% 
    mutate(signif = factor(signif, levels = c("illumina", "longRead", "both"))) %>% 
    ggplot(aes(base_mean_illumina, base_mean_teloprime)) +
...
cdna_exp <- df_plot %>%
    mutate(signif = case_when(
            illumina_transcript & cdna_transcript   ~ "both",
            illumina_transcript               ~ "illumina",
            cdna_transcript                   ~ "longRead",
            TRUE                        ~ "n.s.")) %>% 
    filter(signif != "n.s.") %>%
    mutate(signif = factor(signif, levels = c("illumina", "longRead", "both"))) %>% 
    ggplot(aes(base_mean_illumina, base_mean_cdna)) +
