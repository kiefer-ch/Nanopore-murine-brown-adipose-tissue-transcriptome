---
title: "nanopore iBAT, Illumina reads transcript level, all samples"
...
snakemake <- Snakemake(
    input = list('data/dds_gencode.vM22_transcript.rds'),
    output = list('res/txlevel_all/deseq_txlevel_all.html'),
    params = list(),
    wildcards = list(),
    threads = 1,
    log = list(),
...
# import data
dds <- readRDS(file.path("../data/dds_gencode.vM22_transcript.rds"))
```
...
```{r exportCountMatrices, dependson="annotateCountMatrices"}
# export count matrices
res_folder <- "../res/txlevel_ont" 
...
Some tanscripts in the range 10 to 700 counts show a high (positive) log2FoldChange.
Though there are way less compared to the ont only analysis.
They might be treated as outliers by the lfc shrinking algoryhtm and therefore not shrunken at all.
It is worth investigating their expression (Maybe only one sample with high expression?).

#### Subsetting those with lfc > 5

```{r outliers, dependson="MA-plot"}
order <- colData(dds) %>%
...
pl <- resLFC %>% 
    ...
    tidyr::gather(key = "sample", value = "counts" ,
        ...
        -mgi_symbol, - grouped_biotype, -baseMean, -TPM) %>% 
    ...
    ggplot(., aes(sample, ensembl_transcript_id_version)) +
        geom_tile_interactive(aes(fill = log10(counts), 
            tooltip = sprintf("%s<br>base Mean = %s<br>average TPM = %s", mgi_symbol, round(baseMean, 1), round(TPM, 2)), 
...
Seems as those transcripts are only expressed in one condition and therefore got
assigned a very high log2fc.
...
colData(dds) %>% 
    as_tibble(rownames = "sample") %>% 
    select(sample_id, sample, condition_cage, condition_temp, condition_room, RIN) %>% 
    knitr::kable(format = "html") %>%
    kableExtra::kable_styling()
```
