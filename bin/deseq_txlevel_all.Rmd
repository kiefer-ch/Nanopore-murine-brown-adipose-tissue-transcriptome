...
tpm %>% 
    ...
    annotate_cm(keytype = "ENSEMBLTRANS") %>% 
    rename(transcript_id_ens = "gene_id_ens") %>% 
    write_csv(file.path(res_folder, "txlevel_all_cm_tpm.csv.gz"))

counts(dds, normalized = FALSE) %>% 
    as_tibble(rownames = "gene_id_ens") %>% 
    annotate_cm(keytype = "ENSEMBLTRANS") %>% 
    write_csv(file.path(res_folder, "txlevel_all_cm_cts.csv.gz"))
```
...
```{r runDESeq, dependson="adjustConfounders", echo=TRUE}
# set reference
...
```{r results_shrink, echo=TRUE, dependson="runDESeq"}
...
```{r results_shrink_summary, dependson = "results"}
resLFC <- resLFC %>% 
...
resLFC %>% 
    ...
    write_csv(gzfile(file.path(res_folder, "txlevel_all_de_shrink.csv.gz")))
...
```{r results_noShrink, echo=TRUE, dependson="runDESeq"}
# shrinking log2 fold change
res <- results(dds, 
...
```{r MA-plot_noshrink, dependson = "runDESeq"}
res %>%
    left_join(biotype_groups, by = "gene_biotype") %>% 
...
```{r hclust_noshrink, dependson = "runDESeq"}
# filter significantly regulated genes
hclust_df <- assay(rld) %>%
    as_tibble(rownames = "gene") %>% 
